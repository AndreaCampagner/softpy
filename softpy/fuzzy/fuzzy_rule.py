...
class DNFRule(FuzzyRule):
    ...
    def __init__(self, 
        ...
        for rules in premises:
            if not isinstance(rules, dict):
                raise TypeError("premise should be a list[dict[str, list[FuzzySet]]]")
            for k, l_f in rules.items():
                if not isinstance(k, str):
                    raise TypeError("premise should be a list[dict[str, list[FuzzySet]]]")
                if not isinstance(l_f, list):
                    raise TypeError("premise should be a list[dict[str, list[FuzzySet]]]")
                
                if len(l_f) == 0:
                    raise ValueError("premise should be a non empty list[dict[str, list[FuzzySet]]]")
        ...
        self.__premises: list[dict[str, list[FuzzySet]]] = premises
