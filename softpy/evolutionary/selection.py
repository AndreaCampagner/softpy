...
def tournament_selection(fitness: np.ndarray, tournament_size: int, current_step=None):
    '''
    An optimized implementation of tournament selection.
    '''
    #idx = np.random.choice(range(fitness.shape[0]), tournament_size, replace=True)
    #cambiato da random.choice a random.randint in quanto molto piu` veloce
    idx = np.random.randint(0, fitness.shape[0], size=tournament_size)
    vals = fitness[idx]
...
def stochastic_universal_selection(fitness: np.ndarray, current_step=0):
    '''
